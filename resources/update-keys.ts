...
const NOT_THEME_KEYS = new Set(['workbench.colorCustomizations', 'editor.tokenColorCustomizations'])
...
async function scrapeThemeAvailableKeys() {
  ...
  return [...matches]
    ...
    .filter((key) => !NOT_THEME_KEYS.has(key)) // Remove if its in the blacklist
    .sort()
}
